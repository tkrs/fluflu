...
class ConnectionSpec extends FlatSpec {
  ...
  it should "write successfully" in {
    ...
    val ack  = implicitly[Unpacker[Ack]].apply(ackR).right.get
    assert(ack.ack === "YWJlZjA0Y2UtNWQwOS00NGNjLTg3OTYtMGY5ZTFlNDVhOGY4Cg==")
  }
  ...
  "Constructor" should "throw IOException when its connecting to a server was failed" in {
    ...
    assertThrows[IOException](Connection(inet, Duration.ofSeconds(1), Backoff.fix(Duration.ofMillis(1))))
  }
}
