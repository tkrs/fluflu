...
class MessageUnpackerSpec extends WordSpec {
  ...
  "array" should {
    ...
    "decode nested array" in {
      ...
      val decoded = instance(a).decode[Json]
      val json = io.circe.parser.parse("""[true, {"a":"abc"}, false, [1, 2, 3]]""").toEither
      assert(decoded === json)
    }
  }
  ...
  "nested object" should {
    "be decoded" in {
      ...
      val nestedJson = io.circe.parser.parse("""{"foo": "bar", "buzz": { "x": "X", "y": true, "z": -1 }, "hoge": "fuga", "n": -1152921504606846976, "m": 1.1}""").toEither
  ...
  "map 16" should {
    "be decoded" in {
      ...
      val Right(parsed) = io.circe.parser.parse("""{"a":1.1,"b":2.2,"c":3.3,"d":4.4,"e":5.5,"f":6.6,"g":7.7,"h":8.8,"i":9.9,"j":10.1,"k":11.11,"l":12.12,"m":13.13,"n":14.14,"o":15.15,"p":16.16}""").toEither
